...
describe('engine determinism', () => {
  ...
  it('more laps make a proportionally longer race', () => {
    ...
    // can't flip this scale-sanity check (a 3-lap race runs ~2.3× a 1-lap one; the exact ratio
    // drifts with every balance patch — cooldowns/bursts shift race lengths — so the bound is a
    // loose 2.0×, not a calibration target). Reproducibility itself is covered by the same-seed
    // determinism tests above; this only asserts the rough proportionality holds in aggregate.
    let oneTot = 0;
    let threeTot = 0;
    ...
    expect(threeTot).toBeGreaterThan(oneTot * 2.0);
  });
