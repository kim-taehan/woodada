...
| 단계 | 상태 | 시작일 | 완료일 | 비고 |
|------|------|--------|--------|------|
| 1. 테스트 인프라 | ✅ 완료 | 2026-07-03 | 2026-07-03 | 골든 프레임 해시 게이트 + fixtures/이벤트 헬퍼 + engine-bias `test:stats` 분리(기본 test 5분→5초) + coverage-v8 |
| 2. 타입 분리 | ⏭ 제외 | - | - | types.ts 353줄로 관리 가능 — import churn 대비 이득 없어 계획에서 의도적으로 제외. SkillRuntime 인덱스 시그니처는 명시 필드로 대체 완료 |
| 3. 엔진 시스템 분리 | ✅ 완료 | 2026-07-03 | 2026-07-03 | 캐릭터 패시브 트레이트화(id 하드코딩 10곳 제거) + 순수 헬퍼 승격 + items/ice/decoys 모듈 추출 (RaceEngine 1595→1116줄). 클래스 대신 좁은 deps 모듈 함수 |
| 4. 렌더러 분리 | 대기 | - | - | 다음 라운드 (RaceRenderer 2028줄) |
| 5. 의존성 주입 | ✅ 완료(엔진) | 2026-07-03 | 2026-07-03 | buildSkillContext(deps) + createSkillTestContext — 실제 빌더를 감싼 목 팩토리로 스킬 핸들러 단독 테스트 가능 |
| 6. 통합 테스트 | ✅ 완료(엔진) | 2026-07-03 | 2026-07-03 | 단위 95개(스킬 직접 테스트 포함) + engine-bias 7/7. src/engine 커버리지 96.8% (목표 80% 초과). 전 단계 골든 해시로 비트 동일 결정론 검증 |

---

**최종 업데이트**: 2026-07-03 (엔진 범위 완료 — 렌더러/셸은 다음 라운드)
